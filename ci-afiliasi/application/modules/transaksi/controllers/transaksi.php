<?php
	...
	class Transaksi extends MX_Controller{
		...
		function do_pesan()
		{	
			// if ($this->uri->segment(2)=='do_pesan') {
			// 	redirect('produk');
			// }
			...
			for ($i=0; $i <count($this->input->post('idproduk')); $i++) { 
				...
				if ($id_parent!=null) {
					$datakomisi[$i]=array(
							...
							'userid'=>$id_parent,
							'child_id'=>($this->session->userdata('userid')==null?$this->session->userdata('userid'):0),
							'total_komisi'=>$_POST['qty'][$i]* $_POST['harga'][$i] * 0.2,
							'isactive'=>0,
							'timestamp'=>date('Y-m-d H:i:s')
							);
					$data_komisi=$this->komisi_model->insert_komisi($datakomisi[$i]);	}
				}
			}
